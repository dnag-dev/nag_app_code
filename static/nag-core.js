...
function setupEventListeners() {
    ...
    // IMPORTANT: Don't set up orb click handlers here!
    // Let nag-ui.js handle these to avoid conflicts
    // Instead, just set a flag indicating we should use the handlers from nag-ui.js
    window.nagState.useUiModuleHandlers = true;
...
async function processAudioAndTranscribe() {
    try {
        ...
        if (window.nagState.isSafari) {
            ...
            // Validate we have enough data for Safari
            if (window.nagState.audioChunks.length < 2 && !window.nagState.forceSafariProcessing) {
                window.logDebug("Not enough audio chunks for Safari (need at least 2)");
